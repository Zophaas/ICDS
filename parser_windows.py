...
base = 'D:\\studyyyy\\program\\nlp-winter\\data\\document_parses\\pdf_json\\'

destination = 'D:\\studyyyy\\program\\nlp-winter\\data\\pdf_parsed\\'
...
if merge:
    fm = open(os.path.join(destination, 'merged.txt'), 'w', encoding='utf-8')
...
for filename in tqdm(os.listdir(base)[start_from:start_from+cutoff]):
    ...
    if not merge:
        with open(os.path.join(destination, paper_id+'.txt'), 'w', encoding='utf-8') as f:
            f.write(title)
            f.write('\n')
            f.write(abstract)
            f.write('\n')
